...
namespace Plugin\EccubeUpdater422to423\Controller\Admin;
...
use Plugin\EccubeUpdater422to423\Common\Constant as UpdaterConstant;
...
class ConfigController extends AbstractController
{
    ...
    /**
     * @Route("/%eccube_admin_route%/eccube_updater_422_to_423/config", name="eccube_updater422to423_admin_config")
     * @Template("@EccubeUpdater422to423/admin/config.twig")
     */
     ...
     *
     * @Route("/%eccube_admin_route%/eccube_updater_422_to_423/check_plugin_version", name="eccube_updater422to423_admin_check_plugin_version")
     * @Template("@EccubeUpdater422to423/admin/check_plugin_vesrion.twig")
     */
     ...
     * ファイルの書き込み権限チェックを行う.
     *
     * @Route("/%eccube_admin_route%/eccube_updater_422_to_423/check_permission", name="eccube_updater422to423_admin_check_permission", methods={"POST"})
     * @Template("@EccubeUpdater422to423/admin/check_permission.twig")
     */
     ...
     * 更新ファイルの競合を確認する.
     *
     * @Route("/%eccube_admin_route%/eccube_updater_422_to_423/check_source", name="eccube_updater422to423_admin_check_source", methods={"POST"})
     * @Template("@EccubeUpdater422to423/admin/check_source.twig")
     */
     ...
     * ファイルを上書きする.
     *
     * @Route("/%eccube_admin_route%/eccube_updater_422_to_423/update_files", name="eccube_updater422to423_admin_update_files", methods={"POST"})
     */
    public function updateFiles(Request $request, CacheUtil $cacheUtil)
    {
        ...
        $phpPath = $this->getPhpPath();
        $completeUrl = $this->generateUrl('eccube_updater422to423_admin_complete', [], UrlGeneratorInterface::ABSOLUTE_URL);
        ...
        $commands = [
            ...
            ['eccube:update422to423:plugin-already-installed'],
            ['cache:clear', '--no-warmup'],
            ['cache:warmup', '--no-optional-warmers'],
            ['eccube:update422to423:dump-autoload'],
        ];
...
location.href = '$completeUrl'
     ...
     * 完了画面を表示.
     *
     * @Route("/%eccube_admin_route%/eccube_updater_422_to_423/complete", name="eccube_updater422to423_admin_complete")
     * @Template("@EccubeUpdater422to423/admin/complete.twig")
     */
     ...
     * PluginDir/Resource/config/services.yamlでeccube_update_plugin_422_423_php_pathを定義した場合、こちらが優先されます。
     *
     * @return false|string
     */
    private function getPhpPath()
    {
        $phpPath = $this->eccubeConfig->get('eccube_update_plugin_422_423_php_path');
